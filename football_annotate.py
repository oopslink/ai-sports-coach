...
Arrow types: ball_path (orange, ball trajectory after kick) | run_path (white, player run/movement direction) | pass_direction (cyan, intended pass direction) | correction (green dashed, correct technique/position) | rotation (yellow, body rotation/lean direction)
...
Respond ONLY with valid JSON:
{
  "frames": {
    "1": {
      ...
      "arrows": [
        {"type": "ball_path",   "from_x": 0.48, "from_y": 0.75, "ctrl_x": 0.52, "ctrl_y": 0.60, "to_x": 0.60, "to_y": 0.40, "label": "ball trajectory"},
        {"type": "correction",  "from_x": 0.50, "from_y": 0.50, "to_x": 0.50, "to_y": 0.53, "label": "lean forward"},
        {"type": "rotation",    "from_x": 0.50, "from_y": 0.50, "to_x": 0.53, "to_y": 0.48, "label": "hip rotation"}
      ],
...
FRAME_SYSTEM_PROMPT = """You are a professional football (soccer) coach analyzing football (soccer) training footage for coaching session.
This is an official sports coaching session. Analyze body posture, kicking mechanics, and movement quality only.
...
wrong_contact | ankle_unlocked | poor_follow_through | weak_non_dominant | body_lean |
head_up | off_balance | no_shoulder_check | poor_approach | poor_positioning |
wrong_pass_selection | poor_timing_run | poor_first_touch | wrong_weight_pass |
no_ball_shield | poor_heading | poor_cross_delivery | late_press |
good_technique | good_movement | good_vision

Arrow types: ball_path (orange) | run_path (white) | pass_direction (cyan) |
...
Respond ONLY with valid JSON (no markdown):
{
  ...
  "arrows": [
    {"type":"ball_path","from_x":0.0,"from_y":0.0,"ctrl_x":0.0,"ctrl_y":0.0,"to_x":0.0,"to_y":0.0,"label":"ball path"},
    {"type":"correction","from_x":0.0,"from_y":0.0,"to_x":0.0,"to_y":0.0,"label":"lean forward"}
  ],
...
def _analyze_single_frame(client, frame_path: Path, frame_num: int) -> dict:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": [
                {"type": "text",
                 "text": f"Football training footage frame {frame_num}. Perform biomechanics analysis: locate body parts as spatial coordinates, identify technique issues, and add coaching arrows. Return JSON only."},
                {"type": "image_url",
...
def _analyze_summary(client, frame_results: dict) -> dict:
    ...
    for fn, fdata in sorted(frame_results.items(), key=lambda x: int(x[0])):
        for p in fdata.get("players", []):
            ...
            for part in ["body", "hand_l", "hand_r", "foot_l", "foot_r"]:
                ...
                if it and it not in ("good_technique", "good_movement", "good_vision", ""):
    ...
    prompt = (
        "Based on this football (soccer) session analysis, provide player_analysis and session_summary in JSON.\n"
        "Issues found:\n" + "\n".join(issues_text) +
        '\n\nRespond ONLY with valid JSON:\n'
        '{"player_analysis":{"P1":{"overall_rating":7,"strengths":["中文优点1","优点2"],'
        '"issues":[{"frame":3,"type":"body_lean","body_part":"body","detail":"中文描述"}],'
...
ARROW_STYLES = {
    ...
    "run_path":       {"color": (230, 230, 230), "width": 4, "dash": False},  # 白
    "pass_direction": {"color": ( 40, 210, 210), "width": 4, "dash": False},  # 青
...
def annotate_frame(frame_path: Path, frame_info: dict, out_path: Path):
    ...
    for p in players:
        ...
        if b_it and b_it not in ("good_technique", "good_movement", "good_vision"):
            tag = ISSUE_LABELS.get(b_it, b_it.upper())
            issue_entries.append((bx, by, COLOR["body"], "Body", tag, b_note))

        for pkey, pcol, pname in [
            ("hand_l", COLOR["hand_l"], "Kick Foot"),
            ("hand_r", COLOR["hand_r"], "Stand Foot"),
            ("foot_l", COLOR["foot_l"], "Head"),
            ("foot_r", COLOR["foot_r"], "Hip"),
        ]:
            pd = p.get(pkey) or {}
            it   = pd.get("issue_type", "")
            note = pd.get("issue_note", "")
            if not it or it in ("good_technique", "good_movement", "good_vision"):
                continue
    ...
    for (cx, cy, dot_col, pname, tag, note) in issue_entries:
        is_good = tag in ("GOOD TECH", "GOOD MOVE", "GOOD VISION")
