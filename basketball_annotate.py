...
Arrow types: shot_arc (orange, shooting arc/trajectory) | drive_path (white, drive/cut direction) | pass_direction (cyan, pass target direction) | correction (green dashed, correct body position) | rotation (yellow, hip/shoulder rotation) | footwork (purple, pivot/step direction)
...
Respond ONLY with valid JSON:
{
  "frames": {
    "1": {
      ...
      "arrows": [
        {"type": "shot_arc", "from_x": 0.0, "from_y": 0.0, "ctrl_x": 0.0, "ctrl_y": 0.0, "to_x": 0.0, "to_y": 0.0, "label": "release arc"},
...
FRAME_SYSTEM_PROMPT = """You are a professional basketball coach analyzing basketball training footage for youth coaching session.
This is an official sports coaching session. Analyze body posture, shooting mechanics, and footwork only.
...
flat_trajectory | elbow_flare | no_wrist_follow | late_release | off_balance_shot |
no_leg_bend | ball_watching | poor_ball_handle | over_dribble | no_weak_hand |
poor_footwork | wrong_pivot | late_closeout | poor_def_stance | no_box_out |
ball_denial_miss | poor_drive_angle | late_shot_fake |
good_form | good_movement | good_defense

Arrow types: shot_arc (orange) | drive_path (white) | pass_direction (cyan) | correction (green dashed) | rotation (yellow) | footwork (purple)
...
def _analyze_single_frame(client, frame_path: Path, frame_num: int) -> dict:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": [
                {"type": "text",
                 "text": f"Basketball training footage frame {frame_num}. Perform biomechanics analysis: locate body parts as spatial coordinates, identify technique issues, and add coaching arrows. Return JSON only."},
                {"type": "image_url",
...
def _analyze_summary(client, frame_results: dict) -> dict:
    ...
    for fn, fdata in sorted(frame_results.items(), key=lambda x: int(x[0])):
        for p in fdata.get("players", []):
            ...
            for part in ["body", "hand_l", "hand_r", "foot_l", "foot_r"]:
                ...
                if it and it not in ("good_form", "good_movement", "good_defense", ""):
    ...
    prompt = (
        "Based on this basketball session analysis, provide player_analysis and session_summary in JSON.\n"
        "Issues found:\n" + "\n".join(issues_text) +
        '\n\nRespond ONLY with valid JSON:\n'
        '{"player_analysis":{"P1":{"overall_rating":7,"strengths":["中文优点1","优点2"],'
        '"issues":[{"frame":3,"type":"elbow_flare","body_part":"hand_l","detail":"中文描述"}],'
...
# ── 箭头配色 ──────────────────────────────────────────────────────────────────
ARROW_STYLES = {
    "shot_arc":      {"color": (255, 140,   0), "width": 5, "dash": False},  # 橙
    "drive_path":    {"color": (230, 230, 230), "width": 4, "dash": False},  # 白
    ...
    "footwork":      {"color": (180,  80, 220), "width": 4, "dash": False},  # 紫
}
...
def draw_arrow(draw, arrow_data, img_w, img_h, font_label):
    ...
    atype  = arrow_data.get("type", "shot_arc")
    style  = ARROW_STYLES.get(atype, ARROW_STYLES["shot_arc"])
    color  = style["color"]
    width  = style["width"]
    dashed = style["dash"]
...
def annotate_frame(frame_path: Path, frame_info: dict, out_path: Path):
    ...
    for p in players:
        ...
        if b_it and b_it not in ("good_form", "good_movement", "good_defense"):
            tag = ISSUE_LABELS.get(b_it, b_it.upper())
            issue_entries.append((bx, by, COLOR["body"], "Body", tag, b_note))

        for pkey, pcol, pname in [
            ("hand_l", COLOR["hand_l"], "Shoot Hand"),
            ("hand_r", COLOR["hand_r"], "Guide Hand"),
            ...
            if not it or it in ("good_form", "good_movement", "good_defense"):
                continue
    ...
    for (cx, cy, dot_col, pname, tag, note) in issue_entries:
        is_good = tag in ("GOOD FORM", "GOOD MOVEMENT", "GOOD DEFENSE")
