...
```
Step 1: coach.py         → 提取帧 + GPT-4o 综合分析 → output/report_*.md
Step 2: climbing_annotate.py → 标注每帧问题部位    → output/climbing_annotated/ + output/climbing_report_*.md
Step 3: 向用户输出结构化分析报告（见"报告内容标准"）
```

用户要求 PPT 时额外执行：
```
Step 4: 生成 gen_climbing_ppt.js → node output/gen_climbing_ppt.js
```
...
每个问题必须包含：
- **标注图片**：引用 `output/climbing_annotated/frame_XXX.jpg`（已有部位标注圈）
- **帧号 + 问题类型标签**（见下方标签表）
...
输出：`output/report_YYYYMMDD_HHMMSS.md`、`output/frames/frame_*.jpg`

---

## Step 2：身体部位标注（climbing_annotate.py）

**必须在 coach.py 执行后运行**（依赖 `output/frames/` 目录）：
...
PPT 是高质量呈现方式，用户请求时生成。**直接参考并修改本项目已有的 `output/gen_climbing_ppt.js`**（勿从零生成）。
...
- 标注图路径：`output/climbing_annotated/frame_XXX.jpg`
- 原始帧路径：`output/frames/frame_XXX.jpg`（封面/总结背景图用）
